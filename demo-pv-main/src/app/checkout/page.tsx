...
import { ConfirmacaoPedido } from "@/components/checkout/confirmacao-pedido";
...
export default function CheckoutPage() {
    ...
    // Dados do curso (primeiro item do carrinho)
    const curso = itens.length > 0 ? {
        id: itens[0].id,
        titulo: itens[0].titulo,
        preco: itens[0].preco,
        parcelas: totalParcelas,
        valorParcela: totalPreco / totalParcelas,
        descricao: itens[0].descricao || "",
    } : null;

    // Redirecionar se carrinho estiver vazio
    useEffect(() => {
        if (itens.length === 0 && etapaAtual !== 4) {
            router.push("/carrinho");
        }
    }, [itens, etapaAtual, router]);

    const handleAlunoCriado = (id?: number, dados?: any) => {
        if (id) {
            setAlunoId(id);
            setAlunoData(dados);
            irParaProximaEtapa(); // Vai para Confirmação
        }
    };

    const handlePedidoConfirmado = (id: number) => {
        setPedidoId(id);
        irParaProximaEtapa(); // Vai para Pagamento
    };
    ...
    const handlePagamentoConcluido = () => {
        // Primeiro ir para etapa 4; só depois esvaziar o carrinho.
        // Se limpar antes, itens.some === 0 com etapa ainda em 3 dispara o useEffect e
        // redireciona para /carrinho antes de mostrar o sucesso.
        flushSync(() => {
            setEtapaAtual(4);
        });
        limparCarrinho();
    ...
    // Se carrinho estiver vazio e não for etapa de sucesso, mostra loading
    if (itens.length === 0 && etapaAtual !== 4) {
        return (
            <CheckoutLayout>
        ...
        <CheckoutLayout>
            <div className="max-w-2xl mx-auto w-full">
                <div className="flex items-center justify-between mb-8">
                    ...
                    <CheckoutStepper etapaAtual={etapaAtual as 1 | 2 | 3 | 4} />
                </div>

                {/* Resumo do pedido (visível em todas as etapas) */}
                {itens.length > 0 && etapaAtual !== 4 && (
                ...
                <div className="w-full">
                    ...
                    {/* Etapa 2: Confirmação do Pedido (NOVA) */}
                    {etapaAtual === 2 && alunoId && alunoData && curso && (
                        <ConfirmacaoPedido
                            alunoId={alunoId}
                            alunoData={alunoData}
                            curso={curso}
                            onVoltar={irParaEtapaAnterior}
                            onConfirmado={handlePedidoConfirmado}
                        />
                    )}
                    
                    {/* Etapa 3: Pagamento */}
                    {etapaAtual === 3 && alunoId && pedidoId && (
                        <PagamentoForm
                            alunoId={alunoId}
                            pedidoId={pedidoId}
                            valor={totalPreco}
                            parcelasDisponiveis={totalParcelas}
                            onVoltar={irParaEtapaAnterior}
