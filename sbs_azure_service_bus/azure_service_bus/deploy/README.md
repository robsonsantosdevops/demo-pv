...
O Dockerfile declara `HEALTHCHECK` batendo em `/healthz` via `curl` (intervalo
mais largo porque, após falha, o entrypoint pode ficar até `RESTART_DELAY_SECONDS`
sem servidor HTTP). **Kubernetes ignora o `HEALTHCHECK` do Docker.** Nos manifests
há **`readinessProbe`** em `/ready`; **não** há `livenessProbe` HTTP para não
matar o pod durante a pausa entre tentativas — ver `../k8s/README.md`.
